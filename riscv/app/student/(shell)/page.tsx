...
export default async function StudentDashboardPage() {
  ...
  const username =
    typeof verifyResponse.data?.username === "string"
  ...
  const welcomeName = formatWelcomeName(username);
      ...
      <section className="mt-8 grid gap-5 lg:grid-cols-3">
        {DASHBOARD_CARDS.map((card) => (
          <Link
            key={card.href}
            href={card.href}
            className={`group rounded-[1.75rem] border border-zinc-700/80 p-7 shadow-xl transition duration-200 hover:-translate-y-1 hover:border-zinc-300 ${card.cardClassName}`}
          >
