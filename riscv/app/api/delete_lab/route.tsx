...
import { DeleteLabRequestSchema } from './types';
...
export async function POST(req: NextRequest) {
  ...
  if (verifyResponse.reason === 'reauth_required') {
    return new Response(
      JSON.stringify({
        ...
        message: 'Please sign in again before deleting labs.',
      }),
      {
        status: 401,
  ...
  if (verifyResponse.data.instructor !== true) {
    return new Response(
      JSON.stringify({
        error: 'Forbidden',
        message: 'Only instructors can delete labs',
      }),
      {
        status: 403,
        headers: { 'Content-Type': 'application/json' },
  ...
  try {
    ...
    const deleteResult = await client.query(
      'DELETE FROM labs WHERE uid = $1 RETURNING uid',
      [requestData.uid]
    );

    // Check if lab was found and deleted
    ...
    return new Response(
      JSON.stringify({
        ...
        message:
          'An unexpected error occurred while deleting lab: ' +
